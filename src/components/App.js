...
function App() {
  ...
  const selectedCity = selectedState.city[state.cityIndex];
  const seletedLandmark = selectedCity.landmarks[state.landmarksIndex];
  ...
  return (
    <div id="main">
      <select
        ...
        onChange={(e) =>
          dispatch({ type: "SET_STATE", payload: Number(e.target.value) })
        }
      >
      ...
      <select
        ...
        onChange={(e) =>
          dispatch({ type: "SET_CITY", payload: Number(e.target.value) })
        }
      >
        {selectedState.city.map((items, index) => (
          <option key={index} value={index}>{items.name}</option>
        ))}
      </select>
      <select
        id="landmark"
        value={state.landmarksIndex}
        onChange={(e) =>
          dispatch({ type: "SET_LANDMARK", payload: Number(e.target.value) })
        }
      >
        {selectedCity.landmarks.map((items, index) => (
          <option key={index} value={index}>{items.name}</option>
        ))}
      </select>
